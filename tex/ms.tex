...
\fastspec{} (reference) catalogs, respectively; a unit Gaussian pull
distribution indicates that the two catalogs' quoted uncertainties are
mutually consistent.
...
added in quadrature), \oiii, \hb, and \ha{} against the
...
combine the SV3 bright and dark programs to maximize the matched
sample. (Combining the main bright and dark samples gives nearly
identical statistics, simply with many more objects.)
...
Agreement with \texttt{emlinefit} is excellent for \oii\ and
\ha\ (median offsets of $+0.006$ and $-0.029$~dex and
\texttt{NMAD}\footnote{The normalized median absolute deviation,
...
may contain outliers.} scatter of 0.022 and 0.020~dex), but \oiii{}
and \hb{} show larger offsets ($+0.115$ and $-0.085$~dex,
respectively). Agreement with the DR1 catalog of H.~Zou et~al., (in
preparation) is excellent in median offset ($|\Delta_{\rm
  med}|<0.01$~dex) and scatter ($\mathtt{NMAD}<0.04$~dex) for all four
lines.

The pull distributions (insets) sharpen this picture. Against
\texttt{emlinefit}, the \oiii{} and \hb{} pulls grow increasingly
...
lines are systematically narrower than a unit Gaussian (\texttt{NMAD}
of 0.29--0.71), indicating that the combined quoted flux uncertainties
overestimate the true scatter; since the same \fastspecfit{} reference
does not show this narrowing against \texttt{emlinefit}, the
miscalibration plausibly lies in that catalog's quoted errors rather
than \fastspecfit{}'s, though this combined-pull test alone cannot
