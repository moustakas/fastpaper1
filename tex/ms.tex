...
(\S\ref{sssec:kinematic})---analytic error propagation through every
downstream quantity (stellar mass, K-corrections, EWs, etc.) is
impractical. Monte Carlo resampling instead offers a fast, general way
to sample the multidimensional covariance among all fitted and derived
parameters. By perturbing the input data and refitting, each nonlinear
stage of the pipeline propagates its own uncertainty naturally,
without having to derive a bespoke error-propagation formula for every
derived quantity.
...
Gaussian noise consistent with the input inverse variance,
\begin{equation}
  d_i^{(m)} \sim \mathcal{N}\bigl(d_i,\,I_i^{-1}\bigr),
  \label{eq:mc_draw}
\end{equation}
for realizations $m=1,\ldots,n_\mathrm{monte}$. In this notation,
$d_i$ indexes both spectral pixels and photometric bandpasses. Before
drawing these realizations we inflate the formal inverse variances
with a minimum uncertainty floor, added in quadrature, to guard
against underestimated pipeline errors: 1\% of the flux for the
spectroscopy, and a fixed per-band magnitude floor---2\% for DECam
$grz$ and 5\% for WISE $W1$--$W4$---for the photometry. These draws
are generated once, from a reproducible per-object seed recorded in
the \texttt{SEED} output column---itself derived deterministically
from a single global run seed---and are reused unchanged across every
subsequent fitting stage: the same $n_\mathrm{monte}$ realizations
flow through the velocity-dispersion, continuum, and emission-line
fits below. The inverse variance of any fitted or derived quantity
$\theta$ then follows from the scatter across realizations,
\begin{equation}
  \sigma_\theta^{-2}
  = \bigl[\mathrm{Var}(\theta^{(1)},\ldots,\theta^{(n_\mathrm{monte})})\bigr]^{-1}.
  \label{eq:mc_unc}
\end{equation}
Throughout the pipeline, uncertainties are reported as inverse
variances; parameters that are not measured for a given object, or
that are held fixed at a nominal value, are assigned \texttt{IVAR}=0
...
(equations~\ref{eq:mstar}--\ref{eq:bandshift}), we re-evaluate
...
at their fiducial values and refit only the line
