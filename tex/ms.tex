...
third group anchored on broad \ha.
...
\todo{Need to also explain the doublet ratios like OII\_DOUBLET.}
...
Figure~\ref{fig:example_bgs} shows a low-redshift BGS galaxy at
$z=0.1201$, observed with the DESI fiber centered on the nucleus. Its
high-signal-to-noise spectrum ($\mathrm{S/N}\approx6$--$19$ across the
three cameras) reveals prominent stellar absorption features, narrow
forbidden emission lines, and---most notably---broad Balmer emission
indicative of a type~1 AGN, while strong WISE detections anchor the IR
SED.
...
$z=0.9449$, where the DESI window covers rest-frame
$\sim$1900--5000~\AA{} and brings several emission lines beyond
\oiidoublet{} into view, including H$\gamma$, \hb{}, and
\oiiidoublet{}.  Its faint, blue star-forming continuum
...
\fastspecfit{} cannot evaluate the broad-line selection criteria
(\S\ref{sssec:kinematic}) and correctly retains the narrow-only
kinematic model. The bottom row of Figure~\ref{fig:example_elg} shows
...
broad UV emission-line complexes---Ly$\alpha$+\ion{N}{5}, \ion{C}{4},
...
In this section we present the \fastspecfit{} value-added catalog
(VAC) for DESI DR2. DESI spectra are processed in spectroscopic productions, or
...
(\S\ref{ssec:performance}), our validation of \fastspecfit{}
measurements against independent codes and catalogs
(\S\ref{ssec:validation}), and pipeline diagnostics that use
\fastspecfit{} residuals to characterize DESI calibration errors
(\S\ref{ssec:diagnostics}).
...
\hb, and \ha) exceeds 0.99. A broader criterion is applied to variable
WISE QSO secondary targets; full details are documented in the online
VAC documentation. The original \texttt{Redrock} redshift and
\texttt{ZWARN} bitmask are preserved in the output columns
\texttt{Z\_RR} and \texttt{ZWARN\_RR}. In total, we apply corrected
redshifts to approximately 165,000 targets.

We produce the DR2 VAC (Loa~v2.0) using \fastspecfit{} version~3.6.1
with the \texttt{ftemplates-chabrier-2.2.0.fits} SPS template library
(\S\ref{ssec:templates}), running on the NERSC Perlmutter
supercomputer. The catalog totals approximately 38~million targets
across the DESI main, special, and survey-validation programs
(Table~\ref{table:sample}); the five-year main survey alone
contributes 36.5~million, split between roughly 13~million BGS
targets observed under bright conditions and 23~million LRG, ELG, and
QSO targets observed under dark conditions.

\input{tables/table-sample}

We run \fastspecfit{} on the HEALPix-coadded DESI spectra and organize
the output catalogs identically to how the underlying spectra,
redshift catalogs, and other data products are organized within DESI
data releases. Each output file contains five FITS
extensions---file metadata, target and redshift information, derived
spectrophotometric quantities, emission-line measurements, and (for
\fastspec{} only) best-fitting model spectra---described in full in
Appendix~\ref{sec:appendix_datamodel}. Most users will find it
convenient to work with the merged catalogs, which combine all
per-HEALPix files for a given \texttt{SURVEY} and \texttt{PROGRAM}
into a single file:

\medskip
\noindent\texttt{VACDIR/catalogs/fastspec-SURVEY-PROGRAM.fits}
\medskip

\noindent where \texttt{VACDIR} is the top-level VAC directory. We
will make all catalogs publicly accessible at
\url{https://data.desi.lbl.gov/desi/vac/dr2/fastspecfit/loa/v2.0} upon
the DR2 VAC's public release in 2027. Appendix~\ref{sec:appendix_datamodel}
also describes the per-HEALPix file organization and the
\texttt{nside}~=~1 subdivision used for the largest survey--program
combinations. \fastspecfit{} also produces a companion, photometry-only
\fastphot{} VAC (\S\ref{sec:modes}) for the same DR2 target sample,
released as an independent data product; we describe its data model
and compare its derived stellar masses to the \fastspec{} values in
Appendix~\ref{sec:appendix_fastphot}.

As a concrete example, the snippet below shows how to work directly
with the public per-HEALPix files. It uses
\texttt{fastspecfit.util.radec2pix} to locate the \texttt{nside}~=~1
HEALPixel covering a given sky position, then selects BGS targets
(identified via the \texttt{BGS\_TARGET} bitmask; see \citealt{myers23a}
for a detailed guide to working with DESI targeting bitmasks) with
well-detected \ha, \hb, \niilam, and \oiiilam emission---exactly the
kind of sample needed to build the BPT diagram in
Figure~\ref{fig:bpt} (\S\ref{ssec:reach}). A full, quality-controlled
sample requires the additional cuts described in \S\ref{ssec:cuts}.
...
ra, dec = 150.12, 2.21   # example coordinates
hpix = radec2pix(nside=1, ra=ra, dec=dec)
infile = f'fastspec-loa-main-bright-nside1-hp{hpix:02d}.fits'

meta = fitsio.read(infile, ext='METADATA')
fast = fitsio.read(infile, ext='FASTSPEC')

# select BGS targets with well-detected BPT emission lines
sn = lambda flux, ivar: flux * np.sqrt(ivar)
good = (
    (meta['BGS_TARGET'] != 0) &
    (meta['ZWARN'] == 0) &
    (sn(fast['HALPHA_FLUX'],    fast['HALPHA_FLUX_IVAR'])    > 3) &
    (sn(fast['HBETA_FLUX'],     fast['HBETA_FLUX_IVAR'])     > 3) &
    (sn(fast['NII_6584_FLUX'],  fast['NII_6584_FLUX_IVAR'])  > 3) &
    (sn(fast['OIII_5007_FLUX'], fast['OIII_5007_FLUX_IVAR']) > 3)
)
sample = meta['TARGETID'][good]
\end{verbatim}
...
\todo{Is it worth adding a figure showing the correlation between
  amplitude and flux S/N? Should also discuss possibly comparing
  $\Phi$ to $\Phi_\mathrm{lim}$ from \S\ref{sssec:lines}.}
