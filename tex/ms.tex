...
redshifts; a \ion{Mg}{2}~$\lambda\lambda$2796,2803 doublet fitter
...
[\ion{O}{3}]~$\lambda$4363, and \oiiidoublet{} come from the CHIANTI
atomic database.\footnote{\url{https://db.chiantidatabase.org}}
...
\caption{\todo{Mention the redshift. And maybe an image cutout?}
  ...
  doublet (\ion{Mg}{2}~$\lambda\lambda$2796,2803), a strong forbidden
  doublet (\oiidoublet), a joint Balmer/forbidden group spanning
  H$\gamma$ through \oiiilam, and the merged \ha+\nii+\sii{}
  complex. In each zoom panel, camera-colored pixels mark the
  continuum pixels used for that patch's local linear fit; the dark
  curve shows the combined best-fit line-plus-continuum model; the
  dashed black line and surrounding gray band show the local linear
  continuum and its $\pm1\sigma$ noise envelope; and the labeled S/N
  values report each line's amplitude significance in the line
  masker's own approximate fit. \label{fig:linefit}}
\end{figure*}
...
contains \hb{} together with \oiiidoublet, and another groups
\ha{} with \niidoublet. We activate only patches with at least one line
in the observed spectral range for a given spectrum. Each line carries
an \texttt{isstrong} flag identifying the subset with typical fluxes
reliable enough to measure kinematics; the patch-fitting steps use
only strong lines, while the full list enters the final mask.
...
\oiiilam{} and the narrow Balmer and \ion{He}{1} lines to a second
group anchored on \ha. For each line, we define a pixel window of
$\pm2.5\,\sigma_\mathrm{line}$ centered on the expected observed-frame
wavelength---with a minimum floor of 50~\kms---and identify local
continuum pixels from a surrounding window that we iteratively expand
until at least 11 unmasked pixels are available.  Within each patch we
jointly fit line amplitudes and a linear local continuum; we run two
iterations, updating the line-pixel windows with the best-fit velocity
shifts and widths before the second pass. Second, if any broad Balmer
line falls within the observed spectral range, we repeat the procedure
with the narrow+broad model, initializing the broad Balmer width at
1000~\kms; if any broad Balmer line reaches $S/N > 1.5$ we adopt the
broad-model kinematics for masking, and otherwise retain the
narrow-only widths. Third, we rebuild the pixel mask over the full
...
line-contaminated pixels from the continuum fit; we omit weak
non-strong lines with $S/N < 3.5$ from the final mask to avoid
over-masking. We additionally mask pixels flagged as sky-line
residuals and pixels with zero inverse variance.
...
Figure~\ref{fig:example_elg} shows an ELG at $z \approx 0.8$, where
the DESI spectral window covers rest-frame $\sim$2000--5500~\AA{} and
brings into view several emission lines beyond the \oiidoublet{}
doublet: [\ion{Ne}{3}]~$\lambda3869$, \hb, and \oiii. The faint, blue
star-forming continuum tests the code's ability to recover accurate
line fluxes and a physically meaningful stellar mass in the
...
\ion{Mg}{2} afterburner catalogs distributed alongside each
\texttt{Redrock} output file. For primary QSO targets, we replace the
\texttt{Redrock} redshift when two conditions are met:
\texttt{QuasarNet} independently classifies the object as a QSO, and
the maximum line confidence across six diagnostic transitions
(Ly$\alpha$, \ion{C}{4}, \ion{C}{3}, \ion{Mg}{2}, \hb, and \ha)
exceeds 0.99 for DR2 (0.95 for DR1 and the EDR). A broader criterion
is applied to variable WISE QSO secondary targets; full details are
documented in the online VAC documentation. The original
\texttt{Redrock} redshift and \texttt{ZWARN} bitmask are preserved in
the output columns \texttt{Z\_RR} and \texttt{ZWARN\_RR}. In total,
corrected redshifts are applied to approximately 9,100 targets in the
EDR, 74,000 in DR1, and 169,000 in DR2.
