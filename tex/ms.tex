...
integrated fluxes and equivalent widths for nearly 50 emission lines
spanning the rest-frame UV through near-infrared. We describe the
algorithms, validate the measurements using repeat observations and
comparisons to complementary codes, and present the \fastspecfit{}
...
inverse-variance spectrum, and the \emph{resolution matrix}
$\mathbf{R}$, a band-diagonal, sparse matrix that quantifies the
effective, wavelength-dependent instrumental resolution. $\mathbf{R}$
is a natural output of the optimal, ``spectroperfectionism'' algorithm
of \citet{bolton10a}, which DESI uses to extract one-dimensional
spectra with uncorrelated pixels. It is also a key ingredient of
...
These per-fiber spectra are wavelength- and flux-calibrated, and
coadded across repeat exposures onto a common, 0.8\,\AA{} linear
wavelength grid, expressed as vacuum wavelengths in the Solar System
barycentric frame of reference. The residual wavelength-calibration
uncertainty is $\approx0.025$~\AA{} per sky line, or
$\approx1$\,\kms{} at 7000~\AA{} \citep{guy23a,
  koposov26a}. Comparison of the calibrated spectra against
hydrogen-atmosphere white-dwarf models similarly shows a residual
flux-calibration error of about 2\% over most of the 3600--9800~\AA{}
range (rising to $\approx6\%$ shortward of 3700~\AA), comparable to
the $\pm6\%$ scatter between synthesized DESI spectrophotometry and
imaging photometry (\S\ref{ssec:photometry}) for bright stars
\citep{guy23a}.
...
We organize the 46 emission lines (\S\ref{ssec:emlinelist}) into 23
...
contains \hb\ together with
[O\,\textsc{iii}]~$\lambda\lambda$4959,5007, and another groups
\ha\ with [N\,\textsc{ii}]~$\lambda\lambda$6548,6584.  We activate
only patches with at least one line in the observed spectral range for
a given spectrum. Each line carries an \texttt{isstrong} flag
identifying the subset with typical fluxes reliable enough to measure
kinematics; the patch-fitting steps use only strong lines, while the
full list enters the final mask.
...
\todo{Need a better intro/movitating sentence. Does this section need
  a figure?} The parametric continuum model $C(\lambda)$ may leave
low-level systematic residuals due to imperfect template matching or
...
we use physically motivated \emph{kinematic groups} among different sets
...
into a single group anchored on narrow \ha, keeps \oiii{} as its own
group, and adds a third group for broad Balmer emission anchored on
broad \ha.
...
\ion{Mg}{2}~$\lambda\lambda$2796,2803) act as global kinematic
anchors, the latter with a free amplitude ratio; Table~\ref{table:kinematic}
...
\todo{Need to also document the \texttt{ignore-photometry} mode, the
  ...
  redshifts. Can also run on the per-exposure, per-tile, or
  per-healpix coadds.}
