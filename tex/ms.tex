...
algorithmic stages \fastspec{} executes in turn---fast (preliminary)
fitting in ``patches'' of wavelength (\S\ref{ssec:patches}), stellar
continuum modeling (\S\ref{ssec:continuum}), and emission-line
modeling (\S\ref{ssec:emlines}). We close by describing the physical
...
quasars. \todo{Is it worth noting that the user can easily define
  their own custom line-list? Or maybe mention this flexibility in
  \S\ref{sec:modes}?}
...
pre-fitting pass---the \textit{line masker}---that adapts to each
...
\todo{Should note: The example object (need its redshift) has a
  broad-line component, so the masking is more aggressive.}
...
\caption{\todo{Mention the redshift. And maybe an image cutout?}
  ...
  doublet (\mgiidoublet), a strong forbidden doublet (\oiidoublet), a
  joint Balmer/forbidden group spanning H$\gamma$ through \oiiilam,
  and the merged \ha+\nii+\sii{} complex. In each zoom panel,
...
spectrally adjacent ``patches'', pre-assigned in the configuration
...
contains \hb{} together with \oiiidoublet, and another groups \ha{}
with \niidoublet. We activate only patches with at least one line in
the observed spectral range for a given spectrum. Each line carries an
\texttt{isstrong} flag identifying the subset with typical fluxes
