...
default fitting code, \fastspec, which, by default, jointly fits each
object's DESI spectrum with its corresponding optical/IR broadband
photometry using a nominal (adopted) set of modeling priors. However,
we designed \fastspecfit{} with flexibility and extensibility in mind,
letting users choose different input data and model parameters while
sharing a single, speed-oriented fitting engine.

In practice, \fastspecfit{} exposes this flexibility through
command-line entry points that specify what data to fit. For example,
the \texttt{--ignore-photometry} option drops the broadband photometry
...
lower-redshift BGS targets, this mode is arguably more relevant. Other
comparable options such as \texttt{--no-smooth-continuum} disable the
smooth continuum correction (\S\ref{sssec:smooth}), while
\texttt{--no-broadlinefit} skips the narrow+broad kinematic fitting
(\S\ref{sssec:kinematic}), which may be useful when studying, for
example, individual \hii-region spectra.

Another major fitting mode is \fastphot, which fits only the broadband
photometric SED with the templates convolved to a nominal velocity
dispersion, $\vdisp = 150$~\kms. As for \fastspec, in this mode the
redshift is an input quantity; it is taken from the DESI pipeline
based on the DESI coaddition type---per-exposure, per-night,
cumulative (multi-night), or per-HEALPix---which is detected
...
link lines during fitting. Users can even build a custom sets of SPS
templates with \texttt{bin/build-templates} (\S\ref{ssec:templates})
and use them with any of the available fitting modes with the
\texttt{--templates} command-line argument.
...
Meanwhile, we describe additional fitting modes and capabilities
...
(\S\ref{ssec:templates}) and \texttt{desitarget}~v2.8.0 for target
selection and masking, running on the NERSC Perlmutter
supercomputer. The catalog encompasses the complete five-year main
survey and totals approximately 38~million targets, of which the main
survey---split between roughly 13~million BGS targets observed under
bright conditions and 23~million LRG, ELG, and QSO targets observed
under dark conditions---contributes
36.5~million. Table~\ref{table:sample} summarizes the complete
breakdown by survey and observing program; the corresponding EDR and
DR1 breakdowns appear in Table~\ref{table:sample-edr-dr1}
(Appendix~\ref{sec:appendix_vacs}), where we also describe key
differences in template versions and algorithmic choices relative to
DR2.
