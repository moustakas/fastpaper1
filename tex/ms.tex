...
files---the code processes a typical DESI spectrum in a median 4.8~s
(mean 6.0~s) of single-core time. We use MPI for Python
\citep[\texttt{mpi4py};][]{dalcin21a} to distribute independent
HEALPix files across ranks and Python multiprocessing to parallelize
per-object fits within each rank.  Per-object runtime varies between a
median 3.9~s/object/core for main-survey dark-time targets (LRGs,
ELGs, QSOs) versus 8.1~s/object/core for main-survey bright-time
targets (BGS). This gap in time is primarily due to BGS targets more
frequently triggering a direct velocity-dispersion measurement than
higher-redshift, dark-time targets, which adds a $\chi^2$-scan and
