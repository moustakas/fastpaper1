...
\caption{Comparison of \fastspecfit{} emission-line fluxes with two
  ...
  \citet{zou24b,sui26a}. Columns show, left to right,
  \oii{} ($\lambda3726$+$\lambda3729$), \oiiilam, \hb, and \ha. A
  galaxy is included only if both the \fastspecfit{} and comparison
  flux have $\mathrm{S/N}>3$; no target-class split is applied
  (cf.\ Figure~\ref{fig:compare-mstar-external}). As in
  Figure~\ref{fig:compare-mstar-external}, the logarithmically-scaled
  two-dimensional number density is shown as a color map, with
  contours enclosing 50\%, 75\%, 95\%, and 99.5\% of the joint
  distribution; individual galaxies outside the outermost contour are
  shown as markers, and the dashed line is the 1:1 relation. Each
  panel lists the matched sample size $N$, the median flux offset
  $\Delta_{\rm med}$ (external minus \fastspecfit{}) in dex, and the
  \texttt{NMAD} scatter.  The inset in each panel shows the pull
  distribution (Equation~\ref{eq:pull}), compared to a unit Gaussian
  (black curve).\label{fig:compare-emlines-external}}
\end{figure*}
...
measure, so analyses should carefully assess how they evaluate their
