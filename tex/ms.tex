...
\begin{equation}
  ...
  \label{eq:sfr}
\end{equation}
\noindent in units of $h^{-2}\,\msun$/yr. Additional estimates of the SFR of
course can be inferred from a variety of other tracers, such as the
\ha, \oii, or UV luminosity (where they have been reliably measured
and with appropriate corrections for dust attenuation and other
effects; \citealt{moustakas06b, kennicutt98a}), but \fastspec{} does
not directly report those quantities. Finally, we compute the mean
stellar age as the coefficient-weighted average of the age-bin
midpoints $t_n$ (in Gyr),
\begin{equation}
...
To validate the \fastspecfit{} measurements, we construct overlapping
subsets of the DR2 sample and compare our results against several
independent codes and catalogs. Naturally, these comparisons are not
exhaustive, and because we generate the largest possible comparison
samples, it is possible for differences to be larger or smaller for
narrower subsets of objects. Nevertheless, the following figures
demonstrate that \fastspec{} produces emission-line fluxes (for strong
lines), stellar masses, and velocity dispersions that match reasonably
well to existing independent measurements.
...
Figure~\ref{fig:compare-emlines-external} compares \fastspecfit{}
emission-line fluxes against the DESI \texttt{emlinefit} afterburner
\citep{raichoor23a} and against the DR1 (Iron) CIGALE-based
emission-line fluxes of \citet{zou24b,sui26a}, requiring
$\mathrm{S/N}>3$ on both sides of each comparison. Agreement with
\texttt{emlinefit} is excellent for \oii\ and \ha\ (median offsets of
$+0.006$ and $-0.029$~dex and NMAD scatter of 0.022 and 0.020~dex, for
$N\sim2$--5$\times10^5$ matched galaxies), but \oiiilam\ and \hb\ show
larger offsets ($+0.115$ and $-0.085$~dex, respectively) whose pull
distributions grow increasingly asymmetric toward higher
signal-to-noise---the signature of a multiplicative (flux-scale)
discrepancy between the two codes' line measurements rather than
additive noise. \todo{Confirm physical origin of the
  \oiiilam/\hb\ asymmetry (candidate: differences in stellar
...
\todo{Emission-line comparisons against \texttt{pPXF} and the GSWLC
  catalog of \citet{salim16a} remain for future work.}
...
\todo{We caution against using the SFRs derived by \fastspecfit{}
  because of the lack of ultraviolet photometry and the relatively
  simple modeling priors.}
