...
\caption{Comparison of \fastspecfit{} emission-line fluxes with two
  ...
  The inset in each panel shows the pull distribution,
  $(\mathrm{flux}_{\rm ext}-\mathrm{flux}_{\rm fastspecfit})~/~
  \sqrt{\sigma_{\rm ext}^2+\sigma_{\rm fastspecfit}^2}$, compared to a
  unit Gaussian (black curve), to test whether the two measurements'
  quoted flux uncertainties are mutually
  consistent.\label{fig:compare-emlines-external}}
\end{figure*}
...
%...emission-line fluxes from the DESI \texttt{emlinefit} afterburner
%and the CIGALE-based DR1 catalog of \citet{zou24b,sui26a},
%SED-fitting-based stellar masses from three external catalogs, and
%stellar velocity dispersions from \texttt{pPXF}. Emission-line
%comparisons against \texttt{pPXF} and GSWLC remain for future
%work. For each comparison we select targets with reliable measurements
%in both catalogs and quantify systematic offsets and scatter as a
%function of signal-to-noise ratio, redshift, and (for the stellar-mass
%and SFR comparisons) target class.
...
For all the subsequent comparisons, we match our VAC to each external
...
DESI samples because of our override of some QSO redshifts
(\S\ref{ssec:catalog}).

In Figure~\ref{fig:compare-emlines-external} we compare our
emission-line fluxes for \oiidoublet, \oiiilam, \hb, and \ha{} against
the DESI \texttt{emlinefit} afterburner measurements
\citep{raichoor23a} and against the \texttt{Iron} \citep[DESI
  DR1;][]{desi-collaboration26a} ``Stellar Mass and Emission Line
Catalog'' (\texttt{v1.0}) by \citet{zou24b} (see also H.~Zou et al.,
in
preparation).\footnote{\url{https://data.desi.lbl.gov/doc/releases/dr1/vac/stellar-mass-emline}}

For each matched galaxy we define the pull as

\begin{equation}
\mathrm{pull} = \frac{f_{\rm ext} - f_{\rm
    fastspecfit}}{\sqrt{\sigma_{\rm ext}^2 + \sigma_{\rm
      fastspecfit}^2}},
\label{eq:pull}
\end{equation}

\noindent where $f$ and $\sigma$ are the line flux and its 1$\sigma$
uncertainty in each catalog; a unit Gaussian pull distribution
indicates that the two catalogs' quoted uncertainties are mutually
consistent, while a non-unit width or a skewed/offset distribution
signals under- or overestimated errors, or a systematic bias between
the two measurements.

%DR1 (Iron)

...requiring $\mathrm{S/N}>3$ on both sides of each
comparison. Agreement with \texttt{emlinefit} is excellent for
\oii\ and \ha\ (median offsets of $+0.006$ and $-0.029$~dex and NMAD
scatter of 0.022 and 0.020~dex, for $N\sim2$--5$\times10^5$ matched
galaxies), but \oiiilam\ and \hb\ show larger offsets ($+0.115$ and
$-0.085$~dex, respectively) whose pull distributions grow increasingly
...
measurements rather than additive noise. \todo{Confirm physical origin
  ...
  once resolved with the \texttt{emlinefit} developer.} Agreement with
the Iron CIGALE catalog is excellent in median offset for all four
lines ($|\Delta_{\rm med}|<0.01$~dex), but the pull distributions are
systematically narrower than a unit Gaussian (standard deviations of
0.36--0.94 across the four lines), indicating that the combined quoted
flux uncertainties overestimate the true scatter; since the same
\fastspecfit{} reference does not show this narrowing against
\texttt{emlinefit}, the miscalibration plausibly lies in the CIGALE
