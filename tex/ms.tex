...
\todo{The table should add a comment about the ``UV/QSO''
  ...
  shown here (but used in the next section).}
...
\caption{Illustration of the line-masking algorithm
  (\S\ref{ssec:patches}) for an example BGS target at
  $z=0.3292$. \emph{Top:} the full per-camera DESI spectrum (B, R, Z
  in blue, green, and red, respectively). Overplotted on each fitted
  patch, in the same dark camera colors, is that patch's best-fit
  model---the Gaussian line profile(s) plus local linear continuum
  pedestal. Black boxes and connector lines tie four representative
  patches to the zoom panels below. \emph{Bottom:} the boxed patches,
  left to right---a UV doublet (\mgiidoublet), a strong forbidden
  doublet (\oiidoublet), a joint Balmer/forbidden group spanning
  H$\gamma$ through \oiiilam, and the merged \ha+\nii+\sii{}
  complex. In each zoom panel, camera-colored pixels mark the pixels
  excluded by the final line mask, while light-gray pixels are the
  surrounding continuum used for that patch's local linear fit; the
  dark curve shows the combined best-fit line-plus-continuum model; the
  ...
  masker's own approximate fit. \todo{Consider adding an image
    cutout.} \label{fig:linemasker}}
\end{figure*}
