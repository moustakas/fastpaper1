...
$d_i$ & Observed spectral flux density at pixel $i$ (reused as $d_j$ in \S\ref{sssec:emline_model}) & \S\ref{sssec:fitting} \\
$I_i$ & Inverse variance of spectral pixel $i$ & \S\ref{sssec:fitting} \\
$D_k$ & Observed photometric flux at band $k$ & \S\ref{sssec:fitting} \\
$I_{D,k}$ & Inverse variance of photometric band $k$ & \S\ref{sssec:fitting} \\
...
$\mathbf{b}$ & Stacked, $\sqrt{I}$-weighted spectroscopic$+$photometric data vector (NNLS right-hand side) & \S\ref{sssec:fitting} \\
...
$\eta_f$, $\bar\eta$ & Per-band / scalar aperture-correction factor & \S\ref{sssec:apercorr} \\
$D_f$ & Tractor total-flux photometry in band $f$ & \S\ref{sssec:apercorr} \\
$C_f^\mathrm{fib}$ & Fiber-normalized continuum model, synthesized in band $f$ & \S\ref{sssec:apercorr} \\
...
$s$ & Emission-line width, in \kms{} & \S\ref{sssec:emline_model} \\
$\sigma$ & Emission-line width in log-$\lambda$ units, $\sigma=s/c$ & \S\ref{sssec:emline_model} \\
$\Phi$ & Total integrated emission-line flux & \S\ref{sssec:emline_model} \\
$N$ & Number of emission lines within the observed spectral range (distinct from $N=5$ templates above) & \S\ref{sssec:emline_model} \\
$N_\mathrm{dof}$, $\Delta N_\mathrm{dof}$ & Degrees of freedom / change thereof & \S\ref{sssec:kinematic} \\
$N_\sigma$ & Pixel-window half-width, in units of $\sigma$ & \S\ref{sssec:kinematic} \\
...
\tablecomments{Symbols are grouped by role rather than strictly by order of
  ...
  Section~\ref{sec:modeling}: $C$, $D$, $F$, $M$, $N$, and $\mathcal{R}$/$\mathbf{R}$
