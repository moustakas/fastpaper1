...
\texttt{AGE} / \texttt{AGE\_IVAR}     & Gyr                  & Coefficient-weighted mean stellar age (eq.~\ref{eq:age}) and its inverse variance. \\
\texttt{ZZSUN} / \texttt{ZZSUN\_IVAR} & \nodata              & Logarithmic stellar metallicity relative to solar and its inverse variance (fixed at solar for the current templates; \S\ref{ssec:templates}). \\
\texttt{LOGMSTAR} / \texttt{LOGMSTAR\_IVAR} & $\log(M_\odot)$ & Stellar mass ($h=1$, Chabrier IMF; eq.~\ref{eq:mstar}) and its inverse variance. \\
\texttt{SFR} / \texttt{SFR\_IVAR}     & $M_\odot$\,yr$^{-1}$ & SFR averaged over the most recent 100~Myr (eq.~\ref{eq:sfr}) and its inverse variance. \\
...
\texttt{SMOOTHCORR\_[B/R/Z]}           & \%                   & Median smooth continuum correction relative to data in each camera (\S\ref{sssec:smooth}). \\
\texttt{APERCORR}                      & \nodata              & Median aperture correction factor (\S\ref{sssec:fitting}). \\
